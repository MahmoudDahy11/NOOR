...
class _CreateRoomFormState extends State<CreateRoomForm> {
  ...
  void dispose() {
    ...
    _goalCtrl.dispose();
    _selectedDhikr.dispose();
    _isPublic.dispose();
    _selectedHours.dispose();
    super.dispose();
  }
  ...
  void _submit() {
    ...
    final hours = widget.roomType == RoomType.free ? 0.5 : _selectedHours.value;
    widget.onSubmit(
      CreateRoomParams(
        name: _nameCtrl.text.trim(),
        type: widget.roomType == RoomType.free ? 'free' : 'paid',
        dhikr: dhikr,
        goal: int.tryParse(_goalCtrl.text) ?? 100,
    ...
    return Form(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            FormHandle(
              color: widget.roomType == RoomType.free
            ...
            const SizedBox(height: 20),
            RoomGoalField(controller: _goalCtrl, roomType: widget.roomType),
